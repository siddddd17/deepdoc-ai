    ...
    def load_retriever_from_faiss(self, index_path: str = None):
        ...
        try:
            ...
            self.retriever = FAISS.load_local(faiss_index_path,
                                             embeddings,
                                             allow_dangerous_deserialization=True).as_retriever(search_type = "similarity", search_kwargs={"k":5}) # load retriever from faiss vector store
    ...
    def _build_lcel_chain(self):
        try: 
            if self.retriever is None:
                raise DeepDocException("No retriever set before building chain", sys)
            
            self.log.info(f"contextualize_prompt: {type(self.contextualize_prompt)}")
            self.log.info(f"llm: {type(self.llm)}")
            self.log.info(f"qa_prompt: {type(self.qa_prompt)}")
